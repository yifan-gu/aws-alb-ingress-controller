...
func (c *rulesController) getDesiredRules(ctx context.Context, listener *elbv2.Listener, ingress *extensions.Ingress, ingressAnnos *annotations.Ingress, tgGroup tg.TargetGroupGroup) ([]elbv2.Rule, error) {
	...
	for _, ingressRule := range ingress.Spec.Rules {
		...
		for _, path := range ingressRule.HTTP.Paths {
			...
			elbConditions := buildConditions(ctx, ingressRule, ingressAnnos, path)
			elbRule := elbv2.Rule{
...
func buildConditions(ctx context.Context, rule extensions.IngressRule, ingressAnnos *annotations.Ingress, path extensions.HTTPIngressPath) []*elbv2.RuleCondition {
	var conditions []*elbv2.RuleCondition
	if rule.Host != "" && !ingressAnnos.LoadBalancer.NoHostRouting {
